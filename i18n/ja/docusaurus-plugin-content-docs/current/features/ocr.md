...
OCRを実行するには、スキャンする画面領域を定義する必要があります。エリアセレクターには複数種類の選択ボックスがあります：
...
| <span style={{color: 'deepskyblue'}}>**シアン**</span> | Alt + 左クリック | **排他エリア** — このボックス内にテキストが現れた場合、他の自動（緑）ボックスをすべて無視し、*このボックスの結果のみ*が送信されます。特定のレイアウト、例えば通常の対話エリアに表示される「戦闘後のサマリー」を無視しつつ、左下のチャットをキャプチャするような場合に便利です。 |
| **黒** | Ctrl + Alt + 左クリック | **ブラックホール** — このボックス内で*何らかの*テキストが検出された場合、OCR結果全体が無効化されます。常に表示されるUIコンポーネント（例：HUD要素）があるときにキャプチャを抑制するのに便利です。 |
